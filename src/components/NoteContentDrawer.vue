<template>
    ...
    <div class="flex items-start justify-between">
      <div class="flex-1 min-w-0 mr-3">
        <div class="flex items-center gap-2 mb-1">
          <component :is="getNoteIcon(localNote?.type || 'plain_text')" class="w-5 h-5" :class="getNoteIconColor(localNote?.type || 'plain_text')" />
          <h2 class="text-xl font-semibold text-white truncate">
            {{ localNote?.title || getNoteTypeLabel(localNote?.type || 'plain_text') }}
          </h2>
        </div>
        <p class="text-sm text-gray-400">{{ formatDate(localNote?.updated_at || '') }}</p>
      </div>
      <button
  ...
  <div class="flex-1 overflow-y-auto p-6">
    <div v-if="localNote" class="space-y-4">
      <!-- Public/Shareable badges -->
      <div v-if="localNote.is_public || localNote.is_shareable" class="flex gap-2">
        <span v-if="localNote.is_public" class="inline-flex items-center gap-1 px-3 py-1 bg-green-900/30 border border-green-700 text-green-400 text-xs rounded">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
          </svg>
          Public
        </span>
        <span v-if="localNote.is_shareable" class="inline-flex items-center gap-1 px-3 py-1 bg-blue-900/30 border border-blue-700 text-blue-400 text-xs rounded">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-gray-800 border border-gray-700 rounded-lg p-4">
        ...
        <pre v-else-if="localNote.type === 'songcode'" class="font-mono text-sm text-gray-200 whitespace-pre-wrap break-words overflow-x-auto">{{ localNote.content }}</pre>

        <div v-else-if="localNote.type === 'youtube'" class="space-y-3">
          <div v-if="getYouTubeId(localNote.content ?? '')" class="aspect-video bg-gray-900 rounded overflow-hidden">
            <iframe
              :src="`https://www.youtube.com/embed/${getYouTubeId(localNote.content ?? '')}`"
              class="w-full h-full"
              frameborder="0"
      ...
      <div class="text-xs text-gray-500 space-y-1">
        <p>Created: {{ formatFullDate(localNote.created_at ?? '') }}</p>
        <p>Last updated: {{ formatFullDate(localNote.updated_at ?? '') }}</p>
      </div>
    </div>
...
const looperContent = computed(() => {
  if (localNote.value?.type === 'looper' && localNote.value.data) {
    return localNote.value.data as LooperContent
  }
  return null
})
...
function handleEdit() {
  ...
  drawerStore.push(NoteEditor, {
    note: localNote.value,
    librarySongId: props.librarySongId,
    onSaved: (updatedNote: Note) => {
      localNote.value = updatedNote
      props.onNoteChanged?.()
    },
  })
}
...
async function handleDelete() {
  ...
  try {
    await notesStore.deleteNote(localNote.value.id, props.librarySongId)
